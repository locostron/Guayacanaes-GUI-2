...
export const directories = [
  `${homedir}/.Guayacanes-SPM-023`,
  `${homedir}/.Guayacanes-SPM-023/logs`
];
...
remote.app.setAppUserModelId('wallet.cirq.locostron');

log.debug(`Guayacanes-SPM-023 started...`);
  ...
  const message = (
    <div>
      ...
      <p className={`subtitle ${textColor}`}>
        There&apos;s a new version of CirqWallet available. Would you like to
        download it?
      </p>
    </div>
  );
...
function handleHelp() {
  remote.shell.openExternal('https://chat.cirquity.com/');
}
...
function handleOpen() {
  const options = {
    ...
    filters: [
      {
        name: 'Guayacanaes-GUI-1 File (v0)',
        extensions: ['wallet']
      }
    ]
  };
